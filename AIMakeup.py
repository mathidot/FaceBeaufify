...
class Makeup():
    ...
    化妆器
    '''
    def __init__(self,predictor_path="./data"):
...
def get_faces(im_bgr,im_hsv,temp_bgr,temp_hsv,name):
    rects = detector(im_bgr,1)
    #if len(rects) < 1:
    #    raise NoFace('Too many faces in ' + name)
    return {name:[Face(im_bgr,im_hsv,temp_bgr,temp_hsv,np.array([[p.x, p.y] for p in predictor(im_bgr, rect).parts()]),i) for i,rect in enumerate(rects)]}
...
def main():
    ...
    while True:
        success,im = cap.read()
        if not success:
            raise Exception('Unable to read images from camera')
        cv2.imshow("original",im)

        im_hsv = cv2.cvtColor(im,cv2.COLOR_BGR2HSV)
        temp_bgr,temp_hsv = im.copy(),im_hsv.copy()
        faces = get_faces(im,im_hsv,temp_bgr,temp_hsv,name)
